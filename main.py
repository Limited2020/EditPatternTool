...
def EditPattern(PinName, something, CycleRange, Mode, timemode, MaxReptNum):
    ...
    if Mode == 'Compress Pattern':
        AddReapt(something, timemode, MaxReptNum)
        return
...
def AddReapt(something, timemode, maxReptNum):
    '''tmpstr = open('','r')'''
    # print(os.getcwd())
    # print(something)
        ...
        with open(something) as ATPfile:
            while True:
                ...
                if not Boby_Flag:
                    ...
                    if cmp(line, PrevLine) == 0 and RepeatCnt < int(maxReptNum):
                        RepeatCnt += 1
...
def main4(ATPFiles, CSVFiles, PinName, Mode, TimeMode, MaxReptNum):
    # ATPFiles = []
    # CSVFiles = []
    ...
    for key in CycleRanges.keys():
        ...
        if j >= 0:
            if Mode == 'DSSC Capture':
                EditPattern(PinName, ATPFiles[j], CycleRanges[key], Mode, timemode, MaxReptNum)
            elif Mode == 'DSSC Source':
                EditPattern(PinName, ATPFiles[j], CycleRanges[key], Mode, timemode, MaxReptNum)
            elif Mode == 'CMEM/HRAM Capture':
                EditPattern(PinName, ATPFiles[j], CycleRanges[key], Mode, timemode, MaxReptNum)
